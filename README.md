...
The text data comes from scraped Factbase pages containing Donald Trump Truth Social posts. The scraping script stores post metadata and text in `factbase_truthsocial_texts.csv`, and subsequent preprocessing creates cleaner modeling files.

Important dataset artifacts:

| File | Purpose |
| --- | --- |
| `factbase_truthsocial_texts.csv` | Raw scraped post records from Factbase. |
| `factbase_truthsocial_texts_clean.csv` | Cleaned post-level dataset with parsed dates, URLs, text lengths, word counts, and duplicate flags. |
| `factbase_truthsocial_texts_nlp_ready.csv` | Normalized text prepared for NLP feature extraction. |
| `factbase_truthsocial_texts_sentiment_labeled.csv` | Final labeled dataset used by the logistic regression experiments. |
...
2. The cleaning stage produces `factbase_truthsocial_texts_clean.csv`, including standardized text fields, parsed dates, text length, word count, and duplicate text flags.
...
The saved validation comparison contains 39 custom optimizer runs plus one sklearn reference row in `validation_optimizer_results.csv`. Selected custom configurations are written to `selected_optimizer_configs.csv`, and the final held-out comparison is written to `final_test_optimizer_results.csv`.
...
The validation-based outputs are generated by `optimizer_tradeoff_experiments.py`. The important files are `validation_optimizer_results.csv`, `selected_optimizer_configs.csv`, `final_test_optimizer_results.csv`, and `validation_optimizer_report.json`.
...
![Loss curves by optimizer](optimizer_analysis_outputs/loss_curves_by_optimizer.png)

![Runtime vs macro F1](optimizer_analysis_outputs/runtime_vs_macro_f1.png)

![Learning rate sensitivity](optimizer_analysis_outputs/learning_rate_sensitivity.png)

![Mini-batch size tradeoff](optimizer_analysis_outputs/mini_batch_size_tradeoff.png)

![Stability across seeds](optimizer_analysis_outputs/stability_across_seeds.png)

![L2 interaction plot](optimizer_analysis_outputs/l2_interaction_plot.png)

An animated loss visualization is also saved as `optimizer_analysis_outputs/optimizer_loss_animation.gif`.
...
| --- | --- |
| `prepare_tfidf_dataset.py` | `factbase_truthsocial_texts_nlp_ready.csv`, `factbase_truthsocial_texts_tfidf.npz`, `factbase_truthsocial_texts_tfidf_features.csv`, `factbase_truthsocial_tfidf_vectorizer.joblib`, `factbase_truthsocial_tfidf_config.json` |
| `label_sentiment_dataset.py` | `factbase_truthsocial_texts_sentiment_labeled.csv`, `factbase_truthsocial_texts_sentiment_label_summary.json` |
| `train_sentiment_logreg.py` | `sentiment_logreg_pipeline.joblib`, `sentiment_logreg_report.json`, `sentiment_logreg_test_predictions.csv` |
| `train_custom_optimizers.py` | `optimizer_results.csv`, `optimizer_loss_history.csv`, `optimizer_report.json`, `optimizer_test_predictions.csv`, `optimizer_loss_curves.png` |
| `optimizer_tradeoff_experiments.py` | `validation_optimizer_results.csv`, `validation_optimizer_loss_history.csv`, `validation_optimizer_stability_summary.csv`, `optimizer_seed_stability_summary.csv`, `selected_optimizer_configs.csv`, `final_test_optimizer_results.csv`, `validation_optimizer_report.json`, `results/figures/seed_stability_boxplot.png` |
| `analyze_optimizer_tradeoffs.py` | Summary CSV/Markdown files and figures in `optimizer_analysis_outputs/` |
...
|-- optimizer_loss_history.csv
|-- optimizer_report.json
|-- optimizer_test_predictions.csv
|-- optimizer_loss_curves.png
|-- optimizer_tradeoff_results.csv
|-- optimizer_tradeoff_loss_history.csv
|-- optimizer_stability_summary.csv
|-- optimizer_tradeoff_report.json
|-- validation_optimizer_results.csv
|-- validation_optimizer_loss_history.csv
|-- validation_optimizer_stability_summary.csv
|-- optimizer_seed_stability_summary.csv
|-- selected_optimizer_configs.csv
|-- final_test_optimizer_results.csv
|-- validation_optimizer_report.json
|-- results/
|   `-- figures/
|       `-- seed_stability_boxplot.png
`-- optimizer_analysis_outputs/
    |-- overall_optimizer_summary.csv
    |-- overall_optimizer_summary.md
    |-- scenario_best_results.csv
    |-- scenario_best_results.md
    |-- optimizer_interpretation.md
    |-- analysis_metadata.json
    |-- loss_curves_by_optimizer.png
    |-- runtime_vs_macro_f1.png
    |-- learning_rate_sensitivity.png
    |-- mini_batch_size_tradeoff.png
    |-- stability_across_seeds.png
    |-- l2_interaction_plot.png
    `-- optimizer_loss_animation.gif
```
